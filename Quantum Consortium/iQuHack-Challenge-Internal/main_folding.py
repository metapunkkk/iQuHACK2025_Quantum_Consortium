...
def translate_1qGate(instr):
    ...
    if storage_to_gate:
        lines.append("    # For qubits in storage (indices {}), move to gate". \
                     format(storage_to_gate))
        lines.append("    state.gate[[{}]] = move.Move(state.storage[[{}]])". \
                     format(", ".join(storage_to_gate), ", ".join(storage_to_gate)))
...
def extract_indices(line):
    """
    Given a line of code, extract the indices (as a set of strings)
    from a substring like: indices=[1, 2, 3].
    """
    m = re.search(r'indices=\[([^\]]+)\]', line)
    if m:
        # Split the contents on commas and strip whitespace.
        indices_list = [token.strip() for token in m.group(1).split(',')]
        return set(indices_list)
    return set()

def is_rz_line(line):
    """
    Returns True if the line corresponds to an Rz gate call.
    Adjust the check as needed if your code format changes.
    """
    return "move.LocalRz" in line

def optimize_rz_gates(lines):
    """
    Walks through the list of generated code lines and fuses consecutive
    LocalRz instructions that operate on the same qubit indices, but only
    if no intervening instruction (other than comments or blanks) touches
    any of those same indices.
    """
    optimized_lines = []
    i = 0
    while i < len(lines):
        line = lines[i]
        if is_rz_line(line):
            # Extract the current Rz's rotation angle (phi) and indices.
            phi_match = re.search(r'phi=([\d\.\-eE]+)', line)
            if phi_match:
                combined_phi = float(phi_match.group(1))
            else:
                combined_phi = 0.0

            current_indices = extract_indices(line)
            # We'll merge consecutive safe Rz instructions.
            j = i + 1
            while j < len(lines):
                next_line = lines[j]
                ...
                if is_rz_line(next_line):
                    next_indices = extract_indices(next_line)
                    # Only merge if the indices exactly match.
                    if next_indices == current_indices:
                        next_phi_match = re.search(r'phi=([\d\.\-eE]+)', next_line)
                        if next_phi_match:
                            combined_phi += float(next_phi_match.group(1))
                        j += 1
                        continue
                    else:
                        # A different Rz (with different indices) stops the chain.
                        break
                else:
                    # For any non-Rz line, check if it touches any of these indices.
                    # (This assumes that other instructions also use an "indices=[...]" pattern.)
                    interfering_indices = extract_indices(next_line)
                    if current_indices.intersection(interfering_indices):
                        # An intervening instruction affects one of our qubits,
                        # so we cannot safely merge across it.
                        break
                    else:
                        # This instruction doesn't affect the current qubits,
                        # so we can continue looking ahead.
                        j += 1
                        continue

            # Build a merged Rz instruction line.
            indices_str = ", ".join(sorted(current_indices, key=lambda s: int(s)))
            new_line = "    state = move.LocalRz(atom_state=state,phi={},indices=[{}])".format(combined_phi, indices_str)
            optimized_lines.append(new_line)
            # Skip to the line that broke the chain.
            i = j
        else:
            # Not an Rz instruction; just add the line as is.
