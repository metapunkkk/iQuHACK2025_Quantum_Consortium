...
def translate_init(instr):
    ...
    indices = sorted([str(loc[3]) for loc in instr["init_locs"]], key=int)
...
def translate_1qGate(instr):
    ...
    """
    lines = []
    # Get the gate parameters; default to (1.0, 1.0, 0.0) if not provided.
    ...
    if abs(z1t) > 1e-6:
        ...
        lines.append("    state = move.LocalRz(atom_state=state,phi={},indices=[{}])".format(
            float(z1t), ", ".join(all_gate_indices_sorted)))
    if abs(rxt) > 1e-6:
        lines.append("    # Apply LocalXY with x_exponent={} and axis_phase_exponent={} on indices {}".format(
            float(rxt), -float(np.pi/2), all_gate_indices_sorted))
        lines.append("    state = move.LocalXY(atom_state=state, x_exponent={}, axis_phase_exponent={}, indices=[{}])".format(
            float(rxt), -float(np.pi/2), ", ".join(all_gate_indices_sorted)))
    if abs(z2t) > 1e-6:
        lines.append("    state = move.LocalRz(atom_state=state,phi={},indices=[{}])".format(
            float(z2t), ", ".join(all_gate_indices_sorted)))
...
def translate_rearrangeJob(instr):
    ...
    Qubits that move in the same direction are grouped together and moved in a single move statement.
    ...
    # Sort all index lists numerically.
    storage_to_gate_source = sorted(storage_to_gate_source, key=int)
    storage_to_gate_dest = sorted(storage_to_gate_dest, key=int)
    gate_to_storage_source = sorted(gate_to_storage_source, key=int)
    gate_to_storage_dest = sorted(gate_to_storage_dest, key=int)
    ...
    if storage_to_gate_source:
        ...
        lines.append("    state.gate[[{}]] = move.Move(state.storage[[{}]])  # storage to gate move".format(
            ", ".join(storage_to_gate_dest), ", ".join(storage_to_gate_source)))
    ...
    if gate_to_storage_source:
        ...
        lines.append("    state.storage[[{}]] = move.Move(state.gate[[{}]])  # gate to storage move".format(
            ", ".join(gate_to_storage_dest), ", ".join(gate_to_storage_source)))
    ...
    for sub_inst in instr.get("insts", []):
        ...
        if typ == "activate":
            lines.append("    #  Activate: row_id: {}, col_id: {}".format(
                sub_inst.get("row_id"), sub_inst.get("col_id")))
        elif typ == "deactivate":
            lines.append("    #  Deactivate: row_id: {}, col_id: {}".format(
                sub_inst.get("row_id"), sub_inst.get("col_id")))
        else:
